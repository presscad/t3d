...
int CTexture::LoadGLTextures(char *Filename)									
{
	...
	{
		return false; // 如果沒提供，返回 false
	}
	
	if((pFile = fopen(Filename, "rb")) == NULL) //嘗試打開文件
	{
		
		MessageBox(NULL, "不能夠打開BMP紋理文件!", "打開BMP紋理文件失敗", MB_OK);
		MessageBox(NULL, Filename, "Error", MB_OK);
		return NULL;//打開文件失敗,返回 false
	}
	

	pImage = auxDIBImageLoad(Filename);	//讀取圖像數據並將其返回(使用glaux輔助庫函數auxDIBImageLoad來載入位圖)			
	if(pImage == NULL)	//如果讀取失敗,返回							
		return false;
	
	glGenTextures(1, &m_nTxt); // 創建紋理,告訴OpenGL我們想生成一個紋理名字(如果您想載入多個紋理，加大數字)。
	glPixelStorei (GL_UNPACK_ALIGNMENT, 1);
	glBindTexture(GL_TEXTURE_2D, m_nTxt);// 使用來自位圖數據生成 的典型紋理
	gluBuild2DMipmaps(GL_TEXTURE_2D, 3, pImage->sizeX, 
					  pImage->sizeY, GL_RGB, GL_UNSIGNED_BYTE, pImage->data);
	//pImage->data:告訴OpenGL紋理數據的來源。這裡指向存放在pImage->data中的數據。
	//gluBuild2DMipmaps()代替glTexImage2D(),這樣可以載入任意大小的圖片
	/*
	gluBuild2DMipmaps()與glTexImage2D()的使用方法及區別2008年08月02日 星期六 下午 10:53glTexImage2D()的用法舉例
	
	  glTexImage2D(GL_TEXTURE_2D, //此紋理是一個2D紋理
	  0,                                         //代表圖像的詳細程度, 默認為0即可 
	  3,                                         //顏色成分R(紅色份量)、G(綠色份量)、B(藍色份量)三部分，若為4則是R(紅色份量)、G(綠色份量)、B(藍色份量)、Alpha
		  ...
		  gluBuild2DMipmaps()的用法舉例
			...
			TextureImage[0]->data);             //告訴OpenGL紋理數據的來源,此例中指向存放在TextureImage[0]記錄中的數據
				...
				使用注意事項
				  ...
				  使用glTexImage2D()時所採用的位圖文件分辨率必須為：64×64、128×128、256×256三種格式，如果其他大小則會出現繪製不正常。
					
	*/
	// 設置紋理模式
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_NEAREST);// 雙線過濾
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR_MIPMAP_NEAREST);// 雙線過濾
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,GL_CLAMP_TO_EDGE);//邊緣截取,總是忽略邊界
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,GL_CLAMP_TO_EDGE);//邊緣截取,總是忽略邊界
	glTexEnvf(GL_TEXTURE_ENV,GL_TEXTURE_ENV_MODE,GL_DECAL);	
	if (pImage)		// 紋理是否存在							
	{
		if (pImage->data)	 // 紋理圖像是否存在						
		{
			free(pImage->data);	 // 釋放紋理圖像佔用的內存					
		}

		free(pImage);	// 釋放圖像結構							
	}
	fclose(pFile);// 關閉文件
	return m_nTxt;//返回紋理ID號
