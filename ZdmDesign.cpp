...
void CZdmDesign::InitData()
{
	...
	if(m_currentSchemeIndexs>=0)
	{
		if(myDesingScheme.SchemeDatass[m_currentSchemeIndexs].strDesigngrade!="Ⅲ級鐵路")
			mR=10000;
		else		
			mR=5000;
	...
	for(int i=0;i<7;i++)//縱斷面圖形標注選項
	{
		m_bBZOption[i]=TRUE;
	}

	
	m_bBZOptionText[0]="連續里程";
	m_ItemHeight[0]=20;
	
	m_bBZOptionText[1]="線路平面";
	m_ItemHeight[1]=30;

	m_bBZOptionText[2]="百米標與加標";
	m_ItemHeight[2]=20;
	
	m_bBZOptionText[3]="地面高程";
	m_ItemHeight[3]=45;
	
	m_bBZOptionText[4]="設計坡度";
	m_ItemHeight[4]=30;

	m_bBZOptionText[5]="路肩設計高程";
	m_ItemHeight[5]=45;
	
	m_bBZOptionText[6]="工程地質特徵";
	m_ItemHeight[6]=20;
	...
	LoadDMXGC();//加載地麵線數據
	LoadLJGC();//加載路基數據
	LoadZdmDesignData();//加載縱斷面設計數據
	LoadTunnelData();//加載隧道數據
	LoadBridgeData();//加載橋樑數據
	...
	GetMin_MaxAltitude();// //得到地麵線最大和最小高程值
...
{
	CString tt;
	if(b_HaveCreateFont==FALSE) //如果沒有創建字體
		this->CreateFonts(); //創建字體
	
	dc.SetBkMode(TRANSPARENT ); //背景設置為透明
	CRect rect;	
	GetClientRect(rect);
	dc.FillSolidRect(rect,RGB(255,255,255));  //用畫刷填充
	
	m_H=rect.bottom-20;
	m_W=rect.right;

	int x,y;
	dc.SetTextColor(RGB(0,0,255)); //設置文本顏色
	CFont* oldfont=dc.SelectObject(f); //選擇字體f
	...
	CPen pen(PS_SOLID,1,RGB(220,20,220)); //創建畫筆
	...
	if(m_bHaveGetminMAxY==FALSE)
	{
		...
		m_drwaReginHeight=fabs(m_BeiginLeftY-m_BeiginRightY);//繪圖區域高度
		m_drwaReginWidth=fabs(m_BeiginRightX-m_BeiginLeftX);//繪圖區域寬度
	...
	if(m_drawGrid==TRUE)  //如果繪製背景網格,繪製背景網格線
	{
...
void CZdmDesign::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	dx=m_ZdmStartLC+dx*(1000.0/m_eachx); //當前里程
	...
	if(graph_DesignType==GRAPH_BRIDGE || graph_DesignType==GRAPH_TUNNEL )
	{
		...
		else
		{
			...
			if(graph_DesignType==GRAPH_TUNNEL)
				dlg.m_strTitle="請輸入隧道名稱";
			else if(graph_DesignType==GRAPH_BRIDGE)
				dlg.m_strTitle="請輸入橋樑名稱";
	...
	case 1:
		...
		{
			tt.Format("坡長[%.3f]小於最小坡長【%.3f】,是否取最小坡長?",zdmSlope->L,m_minSlopeLength);
			manswer=MessageBox(tt,"坡長檢查",MB_ICONQUESTION|MB_YESNO);
			if(manswer==7)
				return;
			else
			{
		...
		{
			tt.Format("坡度[%.3f]大於最大坡度【%.3f】,是否取最大坡度",zdmSlope->pd,m_maxSlope);
			manswer=MessageBox(tt,"最大坡度檢查",MB_ICONQUESTION|MB_YESNO);
			if(manswer==7)
				return;
			else
			{
		...
		{
			tt.Format("坡度差[%.3f] 大於最大坡度差%.3f】,是否取最大坡度差",zdmSlope->pddsc,m_maxSlopePddsc);
			manswer=MessageBox(tt,"最大坡度檢查",MB_ICONQUESTION|MB_YESNO);
			if(manswer==7)
				return;
			else
...
void CZdmDesign::DrawGEOfeature(CPaintDC &dc)
{
	...
	tt="扼要填寫沿線各路段重大不良地質現象、主要地層構造等情況";
	tt.Format("select * from zdmSegmentGeoFeature WHERE 方案名稱='%s' \
	...
	while(!m_pRecordset->adoEOF)
	{
		...
		Thevalue=m_pRecordset->GetCollect("分段終點里程"); 
		double m_SegmentendLC=(double)Thevalue;

		Thevalue=m_pRecordset->GetCollect("工程地質特徵"); 
...
void CZdmDesign::LoadDMXGC()
{
	...
	strSql.Format("SELECT *  FROM  T3DLineZxCorrdinate WHERE 方案名稱='%s' ORDER BY 里程 ASC",myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename);
	try
	{
	...
	while (!m_pRecordset->adoEOF)
	{
		...
		Thevalue = m_pRecordset->GetCollect("x坐標"); 
		x=(double)Thevalue;
		Thevalue = m_pRecordset->GetCollect("z坐標"); 
		z=-(double)Thevalue;
	...
	if(m_dmpoints.GetSize()>0)
	{
		...
		tt.Format("SELECT *  FROM  T3DLineZxCorrdinateZDM WHERE 方案名稱='%s' ORDER BY 里程 ASC",myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename);
		m_pRecordset->Open((_bstr_t)tt,(IDispatch*)(theApp.m_pConnection),adOpenDynamic,adLockOptimistic,adCmdText);    
		if(!m_pRecordset->adoEOF)
		{
			Thevalue = m_pRecordset->GetCollect("里程"); 
			m_ZdmStartLC=(double)Thevalue;
			Thevalue = m_pRecordset->GetCollect("Y坐標"); 
...
void CZdmDesign::LoadZdmDesignData()
{
	...
	while (!m_pRecordset->adoEOF)
	{
		...
		Thevalue = m_pRecordset->GetCollect("轉向類型"); 
		zdmSlope->curveStyle=(long)Thevalue;
		
		Thevalue = m_pRecordset->GetCollect("坡度代數差"); 
		zdmSlope->pddsc=(double)Thevalue;

		Thevalue = m_pRecordset->GetCollect("直緩點里程"); 
		zdmSlope->ZHLc=(double)Thevalue;
	
		Thevalue = m_pRecordset->GetCollect("直緩點標高"); 
		zdmSlope->ZH_H=(double)Thevalue;
		
		Thevalue = m_pRecordset->GetCollect("緩直點里程"); 
		zdmSlope->HZLc=(double)Thevalue;
		
		Thevalue = m_pRecordset->GetCollect("緩直點標高"); 
	...
	{
		if(m_dmpoints.GetSize()>0)
			m_ZdmEndLC=m_dmpoints.GetAt(m_dmpoints.GetSize()-1).x; //說明沒有進行縱斷面設計,則初始縱斷面終點里程為地麵線最大里程
		else
			m_ZdmEndLC=0;//表示沒有地麵點
	}
	this->Invalidate();
...
void CZdmDesign::DrawDesignSlope(CPaintDC &dc)
{
	...
	for(i=0;i<ZDmDesignPtsElements.GetSize();i++)
	{
		...
		if(ZDmDesignPtsElements.GetAt(i)->style=="曲線段坐標點")
...
void CZdmDesign::OnZdmMenuSave() 
{
	...
	tt.Format("DELETE FROM ZDmJDCurve WHERE 方案名稱='%s'",myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename);
 	...
 	for(long i=1;i<ZDmJDCurveElements.GetSize();i++)
	{
		...
		try
		{
			...
			if(!SUCCEEDED(hr))
			{
				MessageBox("方案保存失敗!","保存方案",MB_ICONINFORMATION);
				return;
			}
		}
	...
	double MLC;
	tt.Format("SELECT *  FROM  SCHEME_PLANE_CUREVEDATA WHERE 方案名稱='%s' \
		ORDER BY 交點里程 ASC",myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename);
	m_pRecordset->Open((_bstr_t)tt,(IDispatch*)(theApp.m_pConnection),adOpenDynamic,adLockOptimistic,adCmdText);    
	if(!m_pRecordset->adoEOF)
	{
		m_pRecordset->MoveLast();
	...
	tt.Format("UPDATE Scheme SET 起點里程=%.3f,終點里程=%.3f\
		WHERE 方案名稱='%s'",\
...
void CZdmDesign::DrawPlan(CPaintDC &dc)
{
	...
	strSql.Format("SELECT *  FROM  Scheme_plane_CureveData  \
		 WHERE 方案名稱='%s' ORDER BY 直緩點里程 ASC",myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename);
	try
	{
		m_pRecordset->Open((_bstr_t)strSql,(IDispatch*)(theApp.m_pConnection),adOpenDynamic,adLockOptimistic,adCmdText);    
	}
	...
	while (!m_pRecordset->adoEOF)
	{
		...
		Thevalue = m_pRecordset->GetCollect("緩圓點里程"); 
		strSql.Format("%.3f",(double)Thevalue);
		HYLc=(double)Thevalue;
		
		Thevalue = m_pRecordset->GetCollect("圓緩點里程"); 
		strSql.Format("%.3f",(double)Thevalue);
		...
		Thevalue = m_pRecordset->GetCollect("曲線半徑"); 
		R.Format("%.f",(double)Thevalue);
		
		Thevalue = m_pRecordset->GetCollect("轉角"); 
		strSql.Format("%.3f",(double)Thevalue);
		...
		Thevalue = m_pRecordset->GetCollect("曲線長"); 
		L.Format("%.3f",(double)Thevalue);
		

		Thevalue = m_pRecordset->GetCollect("轉向"); 
...
void CZdmDesign::SaveDesignPts()
{
	...
	for(long i=0;i<ZDmJDCurveElements.GetSize();i++)
	{
		...
		if(i==0 || i==ZDmJDCurveElements.GetSize()-1)//起點
		{
	...
	tt.Format("DELETE FROM T3DLineZxCorrdinateZDM WHERE 方案名稱='%s'",myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename);
...
void CZdmDesign::LoadLJGC()
{
	...
	strSql.Format("SELECT *  FROM  T3DLineZxCorrdinateZDM WHERE 方案名稱='%s' ORDER BY 里程 ASC",myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename);
	try
	{
		m_pRecordset->Open((_bstr_t)strSql,(IDispatch*)(theApp.m_pConnection),adOpenDynamic,adLockOptimistic,adCmdText);    
	}
	...
	while (!m_pRecordset->adoEOF)
	{
		...
		Thevalue = m_pRecordset->GetCollect("坐標類型"); 
		if(Thevalue.vt!=VT_NULL) 
		{
...
BOOL CZdmDesign::CHeckDataValid(double SlopeL, float SlopePd, float SlopePddsc)
{
	...
	{
		tt.Format("坡長[%.3f]小於最小坡長【%.3f】",SlopeL,m_minSlopeLength);
		MessageBox(tt,"坡度合法性檢查數據",MB_ICONSTOP);
		return FALSE;
	}
	if(fabs(SlopePd)>m_maxSlope)
	{
		tt.Format("坡度[%.3f]大於最大坡度【%.3f】",SlopePd,m_maxSlope);
		MessageBox(tt,"坡度合法性檢查數據",MB_ICONSTOP);
		return FALSE;
	}
	if(SlopePddsc>m_maxSlopePddsc)
	{
		tt.Format("坡度差[%.3f]大於最大坡度差【%.3f】",SlopePddsc,m_maxSlopePddsc);
		MessageBox(tt,"坡度合法性檢查數據",MB_ICONSTOP);
		return FALSE;
	}
	return TRUE;
...
void CZdmDesign::DrawTunnel(CPaintDC &dc)
{
	...
	for(long i=0;i<m_Tunnelpoints.GetSize();i++)  
	{
		...
		if(m_BeiginLeftX+(x-m_ZdmStartLC)*0.001*m_eachx>=m_BeiginLeftX)
		{
			...
			tt.Format("隧道長度=%.3f",m_Tunnelpoints.GetAt(i)->length); 
...
void CZdmDesign::LoadTunnelData()
{
	...
	tt.Format("SELECT *  FROM  Tunnel WHERE 方案名稱='%s' ORDER BY  隧道起點里程 ASC",myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename);
	try
	{
		m_pRecordset->Open((_bstr_t)tt,(IDispatch*)(theApp.m_pConnection),adOpenDynamic,adLockOptimistic,adCmdText);    
	}
	...
	while (!m_pRecordset->adoEOF)
	{
		...
		Thevalue = m_pRecordset->GetCollect("隧道名稱"); 
		if(Thevalue.vt!=VT_NULL) 
		{
		...
		Thevalue = m_pRecordset->GetCollect("隧道起點里程"); 
		...
		Thevalue = m_pRecordset->GetCollect("隧道終點里程"); 
		...
		Thevalue = m_pRecordset->GetCollect("隧道長度"); 
...
void CZdmDesign::DrawBridge(CPaintDC &dc)
{
	...
	for(long i=0;i<m_Bridgepoints.GetSize();i++)  
	{
		...
		if(m_BeiginLeftX+(x-m_ZdmStartLC)*0.001*m_eachx>=m_BeiginLeftX)
		{
			...
			tt.Format("橋樑長度=%.3f",m_Bridgepoints.GetAt(i)->length); 
...
void CZdmDesign::ModifyPlanDesignData()
{
	...
	strSql.Format("SELECT *  FROM  T3DLineZxCorrdinate WHERE 方案名稱='%s' ORDER BY 序號,里程",myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename);
	try
	{
	...
	strSql2.Format("SELECT *  FROM  T3DLineZxCorrdinateZDM WHERE 方案名稱='%s' ORDER BY 里程",myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename);
	try
	{
	...
	while(!m_pRecordsetZdm->adoEOF)
	{
		...
		Thevalue=m_pRecordsetZdm->GetCollect("Y坐標"); 
		H1=(double)Thevalue;
		...
		if(!m_pRecordsetZdm->adoEOF)
		{
			...
			if(m_pRecordsetZdm->adoEOF)
			{
				...
				if(!m_pRecordsetPm->adoEOF)
				{
					...
					PL1=(double)Thevalue;
					Thevalue=m_pRecordsetPm->GetCollect("X坐標"); 
					x1=(double)Thevalue;
					Thevalue=m_pRecordsetPm->GetCollect("Z坐標"); 
					z1=(double)Thevalue;
					Thevalue=m_pRecordsetPm->GetCollect("序號"); 
					pmID1=(long)Thevalue;
					Thevalue=m_pRecordsetPm->GetCollect("坐標類型");
					if(Thevalue.vt!=VT_NULL) 
					{
					...
					{		
					   ...
					   try
					   {
						   ...
						   if(!SUCCEEDED(hr))
						   {
							   AfxMessageBox("保存失敗!",MB_ICONINFORMATION);
							   return;
						   }
					   }
					   catch(_com_error& e)	
		...
		while(L>=Lc1 && L<=Lc2 && m_pRecordsetPm->adoEOF==FALSE)
		{
			...
			tt.Format("UPDATE T3DLineZxCorrdinate SET Y坐標=%.3f,填挖高=%.3f\
				WHERE 方案名稱='%s' AND 序號=%d",H,H-m_dmh,\
	...
	while(!m_pRecordsetPm->adoEOF)
	{
		...
		Thevalue=m_pRecordsetPm->GetCollect("坐標類型");
		strJdStyle1=Thevalue.bstrVal;
		if(strJdStyle1!="隧道起點" && strJdStyle1!="隧道終點" && strJdStyle1!="隧道中間點" &&\
			strJdStyle1!="橋樑起點" && strJdStyle1!="橋樑終點" && strJdStyle1!="橋樑中間點") 
		{
		...
		if(!m_pRecordsetPm->adoEOF)
		{
			...
			if(strJdStyle2=="隧道起點" || strJdStyle2=="隧道終點" || strJdStyle2=="隧道中間點" ||\
				strJdStyle2=="橋樑起點" || strJdStyle2=="橋樑終點" || strJdStyle2=="橋樑中間點") 
			{
				Thevalue=m_pRecordsetPm->GetCollect("序號");
				long ID=(long)Thevalue;
			

				tt.Format("UPDATE T3DLineZxCorrdinate SET 坐標類型='%s'\
					WHERE 方案名稱='%s' AND 序號=%d",strJdStyle1,\
					myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename,ID);
				hr=theApp.m_pConnection->Execute((_bstr_t)tt,&RecordsAffected,adCmdText); 
			}
...
void CZdmDesign::ModifyPlanDesignData_Tunnel(int mTunnelBridge)
{
	...
	strSql2.Format("SELECT *  FROM  %s WHERE 方案名稱='%s' \
	...
	while(!m_pRecordsetTunnel->adoEOF)
	{
		...
		while(!m_pRecordsetPm->adoEOF)
		{
			...
			if(L>Lc2 && m_bEnd==FALSE)
			{
					...
					Thevalue=m_pRecordsetPm->GetCollect("X坐標"); 
					x1=(double)Thevalue;
					Thevalue=m_pRecordsetPm->GetCollect("Y坐標"); 
					y1=(double)Thevalue;
					Thevalue=m_pRecordsetPm->GetCollect("Z坐標"); 
					z1=(double)Thevalue;
					Thevalue=m_pRecordsetPm->GetCollect("序號"); 
			...
			if(L>=Lc1 && L<=Lc2)
			{
				...
				else
				{
					...
					else if(L==Lc2)
					{
						...
						strJdStyle=mTitle+"中間點";
					

					tt.Format("UPDATE T3DLineZxCorrdinate SET 坐標類型='%s'\
						WHERE 方案名稱='%s' AND 里程=%.3f",strJdStyle,\
...
void CZdmDesign::ModifyPlanDesignDataID(BOOL bModifyAttribute)
{
	...
	strSql.Format("SELECT *  FROM  T3DLineZxCorrdinate WHERE 方案名稱='%s' ORDER BY 里程",myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename);
	try
	{
		m_pRecordsetPm->Open((_bstr_t)strSql,(IDispatch*)(theApp.m_pConnection),adOpenDynamic,adLockOptimistic,adCmdText);    
	}
	...
	while(!m_pRecordsetPm->adoEOF)
	{
		...
		tt.Format("UPDATE T3DLineZxCorrdinate SET 序號=%ld\
			WHERE 方案名稱='%s' AND 里程=%.3f",Nums,\
		...
		if(!m_pRecordsetPm->adoEOF)
		{
			...
			if(strJdStyle1=="橋樑起點")
			{
				...
				{	
					...
					long ID=(long)Thevalue;
					tt.Format("UPDATE T3DLineZxCorrdinate SET 坐標類型='%s'\
						WHERE 方案名稱='%s' AND 序號=%d",strJdStyle2,\
						myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename,ID);
					hr=theApp.m_pConnection->Execute((_bstr_t)tt,&RecordsAffected,adCmdText); 
				}
				
			}				
			else if(strJdStyle1=="隧道起點")
			{
				if(strJdStyle2!="隧道中間點")
				{
					strJdStyle2="隧道中間點";
					Thevalue=m_pRecordsetPm->GetCollect("序號");
					long ID=(long)Thevalue;
...
void CZdmDesign::OnZdmGraphtunnel() 
{
	...
	this->SetWindowText("縱斷面設計→隧道設計【圖形選擇方式】");
}
...
void CZdmDesign::OnZdmGraphbridge() 
{
	...
	m_SelectPtNum=0;
	this->SetWindowText("縱斷面設計→橋樑設計【圖形選擇方式】");
