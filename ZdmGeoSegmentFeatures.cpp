...
void CZdmGeoSegmentFeatures::OnButtonAdd() 
{
	...
	try
	{
		...
		if(!SUCCEEDED(hr))
		{
			MessageBox("增加失敗","增加數據",MB_ICONSTOP);
			return;
		}
	}
	catch(_com_error& e)	
	...
	MessageBox("增加成功","增加數據",MB_ICONINFORMATION);
...
void CZdmGeoSegmentFeatures::InitGrid()
{
	...
	m_grid.SetTextMatrix(0,2,"分段終點里程");
	m_grid.SetTextMatrix(0,3,"工程地質特徵");
...
BOOL CZdmGeoSegmentFeatures::CheckValid()
{
	...
	{
		MessageBox("地質特徵不能為空","數據檢查",MB_ICONSTOP);
		return FALSE;
	}
	
	if(m_SegmentstartLC>m_SegmentendLC)
	{
		MessageBox("分段起始里程不能大於分段終點里程","數據檢查",MB_ICONSTOP);
		return FALSE;
	}
	...
	for(int i=1;i<m_TotalNums;i++)
	{
		...
		if(L1<L2)
		{
			MessageBox("下分段起始里程不能小於上一分段終點里程","數據檢查",MB_ICONSTOP);
			return FALSE;
		}
	}
...
void CZdmGeoSegmentFeatures::OnButtonDelete() 
{
	...
	if(!SUCCEEDED(hr))
	{
		MessageBox("刪除失敗!","刪除數據",MB_ICONSTOP);
		return;
	}	
	LoadData();
...
void CZdmGeoSegmentFeatures::OnButtonEdit() 
{
	...
	strSql.Format("DELETE   from  zdmSegmentGeoFeature where \
		分段起始里程=%.3f AND 分段終點里程=%.3f AND 方案名稱='%s'",\
	...
	try
	{
		...
		if(!SUCCEEDED(hr))
		{
			MessageBox("修改失敗","修改數據",MB_ICONSTOP);
			return;
		}
	}
	catch(_com_error& e)	
	...
	MessageBox("修改成功","修改數據",MB_ICONINFORMATION);
