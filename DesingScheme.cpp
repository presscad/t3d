...
void CDesingScheme::SavePlaneCurveData()
{
	...
	double m_startLC=SchemeDatass[m_currentSchemeIndexs].StartLC; //起點里程
    	
	if(PtS_JD.GetSize()>1) //如果設計交點數大於1
	{
		//計算第1個交點"JD0"的曲線要素
		pTempCurveElements=JDCurveElementss[m_currentSchemeIndexs].GetAt(0);
		pTempCurveElements->ZH=pTempCurveElements->HZ=m_startLC; //緩直點里程 直緩點里程
		...
		pTempCurveElements->JDLC=m_startLC; //交點里程
		pTempCurveElements->ID="JD0"; //交點ID號
		pTempCurveElements->x=PtS_JD.GetAt(0)->x;//交點x坐標
		pTempCurveElements->y=-PtS_JD.GetAt(0)->z;//交點y坐標
		pTempCurveElements->z=PtS_JD.GetAt(0)->y;//交點z坐標
		pTempCurveElements->Cneterx=pTempCurveElements->x;//曲線圓心x坐標
		pTempCurveElements->Cnetery=pTempCurveElements->y;//曲線圓心y坐標
		
		pTempCurveElements->ZH_xy=new  Cordinate;		 //直緩點坐標
		pTempCurveElements->ZH_xy->x=pTempCurveElements->x;
		pTempCurveElements->ZH_xy->y=pTempCurveElements->y;

		pTempCurveElements->HY_xy=new  Cordinate;		//緩圓點坐標
		...
		pTempCurveElements->YH_xy->x=pTempCurveElements->x;//圓緩點坐標
		pTempCurveElements->YH_xy->y=pTempCurveElements->y;

		pTempCurveElements->HZ_xy=new  Cordinate;		
		pTempCurveElements->HZ_xy->x=pTempCurveElements->x;//緩直點坐標
		...
		pTempCurveElements->Alfa=pTempCurveElements->T=pTempCurveElements->L=0; //交點轉角
 
		pTempCurveElements->RoateStyle=pTempCurveElements->E=0; //外矢距
 
		pTempCurveElements->Ly=pTempCurveElements->Jzxc=0; //夾直線
		pTempCurveElements->Dist=pTempCurveElements->fwj=0; //交點偏角
 
		pTempCurveElements->x=PtS_JD.GetAt(PtS_JD.GetSize()-1)->x;//交點x坐標
		pTempCurveElements->y=-PtS_JD.GetAt(PtS_JD.GetSize()-1)->z;//交點y坐標
		pTempCurveElements->z=PtS_JD.GetAt(PtS_JD.GetSize()-1)->y;//交點z坐標
		pTempCurveElements->Cneterx=pTempCurveElements->x; //曲線圓心x坐標
		pTempCurveElements->Cnetery=pTempCurveElements->y;//曲線圓心y坐標
		...
		pTempCurveElements->YH_xy=new  Cordinate;		//圓緩點坐標
		pTempCurveElements->YH_xy->x=pTempCurveElements->x;
		pTempCurveElements->YH_xy->y=pTempCurveElements->y;
		
		pTempCurveElements->HZ_xy=new  Cordinate;		//緩直點坐標
		...
		*/
		...
		for(i=0;i<PtS_JD.GetSize()-1;i++)
		{
			...
			JDCurveElementss[m_currentSchemeIndexs].GetAt(i)->Dist=sqrt(dertE*dertE+dertN*dertN);//交點距離	
		...
		{			
			...
			double xc,yc;
			if(JDCurveElementss[m_currentSchemeIndexs].GetAt(i+1)->RoateStyle==-1)	//左轉
			{
			...
			else if(JDCurveElementss[m_currentSchemeIndexs].GetAt(i+1)->RoateStyle==1)	//右轉
			{
				xc=cos(xita)*ptx+sin(xita)*pty+JDCurveElementss[m_currentSchemeIndexs].GetAt(i+1)->ZH_xy->x;
				yc=sin(xita)*ptx-cos(xita)*pty+JDCurveElementss[m_currentSchemeIndexs].GetAt(i+1)->ZH_xy->y;
			}
			...
			JDCurveElementss[m_currentSchemeIndexs].GetAt(i+1)->HY_xy->x=xc;//緩圓x坐標
			JDCurveElementss[m_currentSchemeIndexs].GetAt(i+1)->HY_xy->y=yc;//緩圓y坐標
			...
			JDCurveElementss[m_currentSchemeIndexs].GetAt(i+1)->YH_xy->x=xc;//圓緩x坐標
			JDCurveElementss[m_currentSchemeIndexs].GetAt(i+1)->YH_xy->y=yc;//圓緩y坐標
	...
	SavePlaneCurveDataToDB(); //保存曲線要素到數據庫中
	
    Save3DlineZX();//在交點間內插點並保存到數據庫和數組中,供程序調用
	
	::MessageBox(NULL,"方案保存成功","保存設計方案!",MB_ICONINFORMATION);
...
CString CDesingScheme::GetLC(double LC)
{
	...
	if(LC==0) //如果里程==0
	{
		tt.Format("K%d+%.3f",0,0); //里程格式轉換
		return tt;		
	}
	
	int L1=int(LC/1000.0); //里程整公里數
	float L2=LC-L1*1000;//剩餘米數
	
	if(L2>99) //如果剩餘米數大於99（表示是3位數）
	{
		tt2.Format("%.3f",L2);//剩餘米數格式轉換
	}
	else if(L2>9)//如果剩餘米數大於9（表示是2位數）
	{
		tt2.Format("0%.3f",L2);//剩餘米數格式轉換
	}
	else //如果剩餘米數是1位數
	{
		tt2.Format("00%.3f",L2);//剩餘米數格式轉換
	}
	tt1.Format("K%d+",L1);//里程整公里數格式轉換
	tt=tt1+tt2; //整體里程格式轉換
	return tt; //返回轉換後里程字符串
}
...
void CDesingScheme::SavePlaneCurveDataToDB()
{
	...
	tt.Format("DELETE FROM  Scheme_plane_CureveData WHERE 方案名稱='%s'",SchemeDatass[m_currentSchemeIndexs].Schemename);
	
	hr=theApp.m_pConnection->Execute((_bstr_t)tt,&RecordsAffected,adCmdText); 
	if(!SUCCEEDED(hr))
	{
		AfxMessageBox("刪除數據失敗!",MB_ICONINFORMATION);
		return;
	}
	
	tt.Format("DELETE FROM  Scheme_plane_CureveData_XY WHERE 方案名稱='%s'",SchemeDatass[m_currentSchemeIndexs].Schemename);
	hr=theApp.m_pConnection->Execute((_bstr_t)tt,&RecordsAffected,adCmdText); 
	if(!SUCCEEDED(hr))
	{
		AfxMessageBox("刪除數據失敗!",MB_ICONINFORMATION);
	...
	try
	{
		...
		if(!SUCCEEDED(hr))
		{
			AfxMessageBox("方案保存失敗!",MB_ICONINFORMATION);
			return;
		}
	}
...
void CDesingScheme::Save3DlineZX()
{
	...
	for (long i=0;i<JDCurveElementss[m_currentSchemeIndexs].GetSize();i++)
	{
		...
		else if(i>0 && i<JDCurveElementss[m_currentSchemeIndexs].GetSize()-1)//當前交點i不是方案的最後一個交點,有緩和曲線段,曲線段
		{
...
void CDesingScheme::Save3DlineZXCorrdinateToDB()
{
	...
	m_pRecordset.CreateInstance("ADODB.Recordset");   
	tt.Format("DELETE FROM  T3DLineZxCorrdinate WHERE 方案名稱='%s'",SchemeDatass[m_currentSchemeIndexs].Schemename);
...
void CDesingScheme::Get3DLineModel(double x1, double y1, double z1,double x2, double y2, double z2, float fRailwayWidth, float LjWidth, float h_FromGmToLj,float mWidthGuiMianToLujian,float mAngleLujinaToBianPo,CString strJDstyle,CString strJDstyleNext,long index,double mLC)
{
	...
	//fRailwayWidth：單線地段道床頂面寬度
	float L1=fRailwayWidth/2.0;

    //根據線路前後中心線的三維坐標計算鐵軌頂面左右點的x,y,z坐標
	...
	//根據線路前後中心線的三維坐標計算道床頂面左右點的x,y,z坐標
	...
	//根據線路前後中心線的三維坐標計算碴腳左右點的x,y,z坐標 LjWidth=碴肩至碴腳的水平距離
	...
	//根據線路前後中心線的三維坐標計算計算路基面左右點的x,y,z坐標
	pt->TW_left=GetTW(pt->x1, pt->z1, pt->y1);//計算左邊坡的填挖類型
	pt->TW_right=GetTW(pt->x2, pt->z2, pt->y2);//計算右邊坡的填挖類型
 	pt->mAngle=mangle; //
	PtS_RailwayLjToBP3D[m_currentSchemeIndexs].Add(pt);//存儲邊坡左面側的填挖類型
	...
	//4.求左右側水溝交點坐標
		...
		*/
		...
		if(pt->TW_left==-1)		//路塹
			ptt->bhaveSuiGou_L=TRUE; //表示左側有水溝(邊坡類型為路塹的情況下設置水溝)
		else
		{
			ptt->bhaveSuiGou_L=FALSE;  //表示沒有水溝
			//沒有水溝的情況下,水溝4,5點與0點重合
		...
		if(pt->TW_right==-1)		//路塹	
			ptt->bhaveSuiGou_R=TRUE; //表示右側有水溝(邊坡類型為路塹的情況下設置水溝)
		else
		{
			ptt->bhaveSuiGou_R=FALSE;//表示沒有水溝
			//沒有水溝的情況下,水溝4,5點與0點重合
		...
		PtS_PaiSuiGou[m_currentSchemeIndexs].Add(ptt); //將計算的水溝點坐標存儲在PtS_PaiSuiGou[]數組中
		
		if(strJDstyle!="橋樑中間點" && strJDstyle!="隧道中間點" ) //如果線路中心點類型不是"橋樑中間點"和"隧道中間點"
		{	
			
			if(strJDstyle=="隧道終點" && strJDstyleNext=="橋樑起點" ) //橋樑隧道直接相連,不設置邊坡
			{	
				PLuQianHuPo ptt;
				ptt= new LuQianHuPo;
				ptt->Huponums_L =0; //左側邊坡數為0
				ptt->Huponums_R =0; //右側邊坡數為0
				ptt->TW_left=99; //表示不需要設置邊坡
				ptt->TW_right=99;//表示不需要設置邊坡
				ptt->strJDStyle=strJDstyle; //交點類型
				ptt->Lc=mLC; //交點里程
				PtS_HuPo[m_currentSchemeIndexs].Add(ptt);//橋隧間點
			}
			else //正常邊坡處
			{
				L1=fRailwayWidth/2.0+mWidthGuiMianToLujian+LjWidth+SuiGou_b1+SuiGou_b3;
		...
		else //如果線路中心點類型是"橋樑中間點"或隧道中間點",則不需要設置邊坡
		{
			PLuQianHuPo ptt;
			ptt= new LuQianHuPo;
			ptt->Huponums_L =0; //左側邊坡數為0
...
//根據線路前後中心線兩點的三維坐標計算對應的路基斷面左右點的x,y,z坐標
...
{
	float mangle;
	if(fabs(dx)<=0.000001)  //如果前後線路中線點的X坐標之差<=0.000001,則認為兩點組成的直線平行於Y軸
	{
		if(dz<0) 
			mangle=PAI/2.0;   //如果dz<0,直線角度=90
		else
			mangle=3/2.0*PAI;   //如果dz<0,直線角度=270
	
		//計算線路中線當前點(x1,y1,z1)對應的左側點三維坐標
		*x11=x1-L*sin(mangle);
		*y11=y1;	
		*z11=z1;
		
		//計算線路中線下一點(x2,y2,z2)對應的左側點三維坐標
		*x21=x2-L*sin(mangle);
		*y21=y2;	
		*z21=z2;
		*angle=mangle;
	}  
	else //根據fabs(dz/dx)值計算前後線路中線點構成的直線角度
	{
	...
	//計算線路中線當前點(x1,y1,z1)對應的右側點三維坐標
	*x12=2*x1-*x11;
	*y12=2*y1-*y11;
	*z12=2*z1-*z11;
	
	//計算線路中線下一點(x2,y2,z2)對應的右側點三維坐標
	*x22=2*x2-*x21;
	*y22=2*y2-*y21;
	*z22=2*z2-*z21;
...
{
	int mTW;
	double tx1=x+theApp.m_DemLeftDown_x; //將x,y坐標轉換為大地坐標
	double  ty1=-z+theApp.m_DemLeftDown_y;
	if(tx1>theApp.m_DemRightUp_x)	//防止x坐標超出邊界	
		tx1=theApp.m_DemRightUp_x;
	if(ty1>theApp.m_DemRightUp_y)	//防止y坐標超出邊界		
		ty1=theApp.m_DemRightUp_y;
	float mh=m_demInsert.GetHeightValue(tx1,ty1,2); //從DEM中內插計算地麵點高程
	if(fabs(H-mh)<=0.001) //如果路基麵點的高程與垂直正下方的地麵點高程之差<=0.001
		mTW=0; //認為是填挖平衡點(即填挖0點)
	else if(H<mh) 
		mTW=-1;  //<垂直正下方的地麵點高程,為路塹
	else
		mTW=1;  //>垂直正下方的地麵點高程,為路堤

	return mTW;  //返回計算的邊坡填挖類型
...
void CDesingScheme::GetBpJD(float H0, float Afla, float mangle, double x0, double z0, int bsignTW, int mLeftRight, double *tx, double *ty, double *tz)
{
	...
	while(fabs(h1-hNew)>DerC) 
	{
		...
		hNew=GetH(L,Afla,mangle,x0,z0,mLeftRight,&xx,&zz); //對應的地面高程
...
void CDesingScheme::Get3DLineModelLast(double x1, double y1, double z1, double x2, double y2, double z2, float fRailwayWidth, float LjWidth, float h_FromGmToLj, float mWidthGuiMianToLujian, float mAngleLujinaToBianPo,CString strJDstyle,CString strJDstyleNext,long index,double mLC)
{
	...
	if(strJDstyle!="橋樑中間點" && strJDstyle!="隧道中間點" )
	{	
		GetDMJD(x1,y1,z1,x2,y2,z2,L1,pt->y1,pt->x1, pt->z1,pt->TW_left,-1,\
			ptt->SuiGouL[4].x,ptt->SuiGouL[4].y,ptt->SuiGouL[4].z,\
			ptt->SuiGouL[5].x,ptt->SuiGouL[5].y,ptt->SuiGouL[5].z,mLC,strJDstyle);
...
void CDesingScheme::GetBirdgeLGcordinate(float BridgeLGJJ)
{
	...
	for (long i=0;i<PtS_RailwayLjToBP3D[m_currentSchemeIndexs].GetSize()-1;i++)
	{
		...
		m_style=PtS_3DLineZX[m_currentSchemeIndexs].GetAt(i)->strJDStyle;//交點類型
		//如果交點類型是"橋樑起點,橋樑終點"或"橋樑中間點",表示該點是發屬於橋樑部分
		if(m_style=="橋樑起點" || m_style=="橋樑終點" || m_style=="橋樑中間點") 
		{	
			...
			if(m_style=="橋樑起點") //如果是"橋樑起點"
			{
				//計算橋樑信息
				tempBridgeInformation=new TunnnelBridgeInformation;
				tempBridgeInformation->startIndex=i+1; //橋樑起點的數據起點索引
				tempBridgeInformation->StartLC=PtS_3DLineZX[m_currentSchemeIndexs].GetAt(i)->Lc; //橋樑起點里程
				
				tempBridgeInformation->StartLeftXYZ.x=pt->x1;//橋樑起點左側的x,y,z坐標
				tempBridgeInformation->StartLeftXYZ.y=pt->y1;
				tempBridgeInformation->StartLeftXYZ.z=pt->z1;
				tempBridgeInformation->StartRightXYZ.x=pt->x2;//橋樑起點右側的x,y,z坐標
				tempBridgeInformation->StartRightXYZ.y=pt->y2;
				tempBridgeInformation->StartRightXYZ.z=pt->z2;
				
			}
			else if(m_style=="橋樑終點")//如果是"橋樑終點"
			{
				tempBridgeInformation->endIndex=i; //橋樑終點的數據起點索引
				tempBridgeInformation->EndLC=PtS_3DLineZX[m_currentSchemeIndexs].GetAt(i)->Lc;//橋樑終點里程
				
				tempBridgeInformation->EndLeftXYZ.x=pt->x1;//橋樑終點左側的x,y,z坐標
				tempBridgeInformation->EndLeftXYZ.y=pt->y1;
				tempBridgeInformation->EndLeftXYZ.z=pt->z1;
				tempBridgeInformation->EndRightXYZ.x=pt->x2;//橋樑終點右側的x,y,z坐標
				tempBridgeInformation->EndRightXYZ.y=pt->y2;
				tempBridgeInformation->EndRightXYZ.z=pt->z2;
				BridgeInfor[m_currentSchemeIndexs].Add(tempBridgeInformation); //存儲橋樑信息
			}
			
			//計算(x1,y1,z1)t (x2,y2,z2)水平距離,+0.0001是為了防止計算誤差
			float length=GetDistenceXY(x1,z1,x2,z2)+0.001;
			int nums=(int)((length+dx)/BridgeLGJJ);//根據兩點距離和橋欄杆間距計算應設置的橋欄杆數
			for(int j=1;j<=nums;j++)
			{
				...
				PtS_BridgeLG3D[m_currentSchemeIndexs].Add(pt);//存儲橋欄杆坐標數據
			}
			dx+=length-nums*BridgeLGJJ; //剩餘距離,如果<=0.01,則認為為0
			if(dx<=0.01) dx=0;

			if(m_style=="橋樑終點") //如果是"橋樑終點"
			{
				pt=new Railway3DCordinate; 
				pt->x1 =-1;   //=-1表示當前橋樑結束標誌
				pt->y1 =-1;
	...
	for (long i=0;i<PtS_RailwayLjToBP3D[m_currentSchemeIndexs].GetSize()-1;i++)
	{
		...
		{	
			...
			if(m_style=="橋樑起點" && i>0 ) //如果是"橋樑起點"
			{
					...
					int Num=templength/BridgeQDJJ; //根據兩點距離和橋墩間距計算應設置的橋墩數
				
					for(int m=1;m<=Num;m++)
					{
						pt=new Railway3DCordinate;
						
						//線性內插計算設置橋墩點的左側橋面對應的三維坐標
						pt->x1=x1+(i*BridgeQDJJ)/templength*(x2-x1);
						pt->y1=y1+(i*BridgeQDJJ)/templength*(y2-y1);
						pt->z1=z1+(i*BridgeQDJJ)/templength*(z2-z1);
						
						
						//線性內插計算設置橋墩點的右側橋面對應的三維坐標
						pt->x2=x3+(i*BridgeQDJJ)/templength*(x4-x3);
						pt->y2=y3+(i*BridgeQDJJ)/templength*(y4-y3);
						pt->z2=z3+(i*BridgeQDJJ)/templength*(z4-z3);

						//根據DEM內插計算左側橋面對應點的地面高程
						float h1=m_demInsert.GetHeightValue(pt->x1+theApp.m_DemLeftDown_x,-pt->z1+theApp.m_DemLeftDown_y,2);
						
						//根據DEM內插計算右側橋面對應點的地面高程
						float h2=m_demInsert.GetHeightValue(pt->x2+theApp.m_DemLeftDown_x,-pt->z2+theApp.m_DemLeftDown_y,2);
						float H=pt->y1-h1; //橋墩高度(根據左側橋麵點計算)
						if(H<(pt->y2-h2)) //如果H<pt->y2-h2(根據右側橋麵點計算的高度)
							H=pt->y2-h2;  //重新計算橋墩高度			
						pt->QDHeight=H;//橋墩高度
						/*
						上述代碼段說明,橋墩高度取左右側計算的最大高度.例如左側計算H=4.56,右側計算後得H=4.78,則取H=4.78
						*/

						PtS_BridgeQD3D[m_currentSchemeIndexs].Add(pt); //存儲橋墩三維坐標
						bFind=TRUE; //找到橋樑
						
					}
					length=templength-BridgeQDJJ*Num;//計算剩餘距離
					if(fabs(length)<=0.001) //如果剩餘距離絕對值<=0.001,則認為為,不再設置橋墩
						length=0;
				
			}
			else  //如果是"橋樑終點"或"橋樑中間點"
			{
				...
				length+=templength; //當前未設置橋墩距離累計
				if(length>=BridgeQDJJ) //如果兩點距離大於橋墩間距,表示需要在兩點間設置橋墩
				{
					pt=new Railway3DCordinate;
					...
					length=length-BridgeQDJJ;//計算新的剩餘距離length
					if(fabs(length)<=0.001)//如果剩餘距離絕對值<=0.001,則認為為,不再設置橋墩
						length=0;
					bFind=TRUE; 
				}
...
{
	HANDLE   filehandle=::LoadImage(NULL,BitmapFilePath,IMAGE_BITMAP,0,0,LR_LOADFROMFILE);  //加載影像文件 
	if(filehandle!=NULL)   //如果加載成功
	{   
		CBitmap   bmp;   
		if(bmp.Attach(filehandle))   
		{   
			BITMAP   bmpInfo;   
			bmp.GetBitmap(&bmpInfo);   //獲取資源位圖信息
			CDC   dcMemory;   
			dcMemory.CreateCompatibleDC(pDC);   //創建一個與特定設備場景一致的內存設備場景
			dcMemory.SelectObject(&bmp);   //選擇bmp對像到指定的設備上下文環境中,該新對像替換先前的相同類型的對象
			//設置指定設備環境中的位圖拉伸模式. HALFTONE：將源矩形區中的像素映射到目標矩形區的像素塊中，
			//覆蓋目標像素塊的一般顏色與源像素的顏色接近。
			
			//StretchBlt():源矩形中複製一個位圖到目標矩形,必要時按目前目標設備設置的模式進行圖像的拉伸或壓縮。
							...
							double tx0,double ty0,double tz0,double tx1,double ty1,double tz1,double mLC,CString strJDstyle)
{
	...
	float m1=1; //1級邊坡坡率
	float m2=1.5;//2級邊坡坡率
	float hqm=0.75; //護牆的坡率
	
	
	//根據規範設置的路堤護坡數值
	float Lt_h1=6;  //1級邊坡坡高
	float Lt_h2=12;//2級邊坡坡高
	float Lt_m1=1.5; //1級邊坡坡率
	float Lt_m2=1.75;//2級邊坡坡率
	float Lt_b=2.0; //邊坡平台寬度
	
	float m_threshodld=0.001;  //設定的閾值
	...
	if(LeftRight==-1) //左側邊坡
	{
		if(TW==-1) //如果是路塹
		{
			...
			Afla=atan(1.0/m1);
			Get3DCorrdinate(x1,y1,z1,x2,y2,z2,dx,dz,L1,&x11,&y11,&z11,&x12,&y12,&z12,&x21,&y21,&z21,&x22,&y22,&z22,&mangle);
			hNew=GetH(h1*m1,Afla,mangle,x11,z11,LeftRight,&xx,&zz);//計算對應地麵點高程值
		
			//如果根據設置的1級邊坡高度(h1)和1級邊坡坡率(m1),計算的地麵點高程與
			//邊坡點和坡高的高差絕對值<=設定的閾值m_threshodld
			if(fabs(hNew-h0-h1)<=m_threshodld) 
			{
				ptt->Huponums_L =1; //左側只有1級邊坡,邊坡數為1
				//記錄左側各邊坡點的坐標和相關屬性值
				...
				ptt->HuPo_L[0].h=8; //邊坡的坡高
				ptt->HuPo_L[0].m=1; //邊坡的坡率
				ptt->HuPo_L[0].style=0; //邊坡的類型(護坡)

				PtS_HuPo[m_currentSchemeIndexs].Add(ptt); //存儲邊坡數據
			}
			else if(hNew-h0-h1<0) //如果之差小於0,表示設置1級坡高h1時,坡腳點已經進入地面以下了,按坡高h1設置大了,即只設置<h1的的坡高就可以了
			{
			...
			//如果之差大於0,表示設置1級坡高h1時,坡腳點未與地面相交,需要繼續設置二級邊坡
			else if(hNew-h0-h1>0)
			{
				...
				//如果根據設置的2級邊坡高度(h2)和2級邊坡坡率(m2),計算的地麵點高程與
				//邊坡點和坡高的高差絕對值<=設定的閾值m_threshodld
				if(fabs(hNew-h0-h1-h2)<=m_threshodld) 	
				{
					//存儲2級邊坡的數據

					ptt->Huponums_L =2; //左側邊坡數為2
					
					//2級邊坡0點的坐標=1級邊坡2點的坐標(即2級邊坡的1級邊坡的2點)
					...
					ptt->HuPo_L[1].h=12;//2級邊坡的坡高
					ptt->HuPo_L[1].m=1;//2級邊坡的坡率
					ptt->HuPo_L[1].b=b2;//2級邊坡的平台寬度
					ptt->HuPo_L[1].style=0; //邊坡的類型(護坡)

					PtS_HuPo[m_currentSchemeIndexs].Add(ptt);//存儲邊坡數據
				}
				//如果根據設置的2級邊坡高度(h2)和2級邊坡坡率(m2),計算的地麵點高程與
				//邊坡點和坡高的高差<0,表示設置2級坡高h2時,
				//坡腳點已經進入地面以下了,按坡高h2設置大了,需要重新計算應該設置的2級坡高
				else if(hNew-h0-h1-h2<0)
				{
					Afla=atan(1.0/m2);
					GetBpJD(h0+h1, Afla, mangle, x11, z11,TW,-1,&xx,&yy,&zz);//計算邊坡點與地麵線的交點坐標 
					ptt->Huponums_L =2; //左側邊坡數為2
					...
					ptt->HuPo_L[1].h=yy-h0-h1;//2級邊坡的坡高
					ptt->HuPo_L[1].m=1; //2級邊坡的坡率
					ptt->HuPo_L[1].b=0;//2級邊坡的平台寬度=0(直接與地面相交)
					ptt->HuPo_L[1].style=0;//邊坡的類型(護坡)
				...
				//如果根據設置的2級邊坡高度(h2)和2級邊坡坡率(m2),計算的地麵點高程與
				//邊坡點和坡高的高差>0,表示設置2級坡高h2時,
				//坡腳點仍高於地面,則取消2級邊坡,在1級邊坡上直接設置護牆
				else if(hNew-h0-h1-h2>0)
				{

					float hh=5; //護牆初始最小高度=5
					float hNew2=999;
					...
					ptt->HuPo_L[1].h=hh;//2級邊坡的坡高
					ptt->HuPo_L[1].m=1;//2級邊坡的坡率
					ptt->HuPo_L[1].b=0;//2級邊坡的平台寬度
					ptt->HuPo_L[1].style=1;//邊坡的類型(護牆)
					...
					ptt->HuPo_L[2].h=hNew2-(h0+h1+hh);//3級邊坡的坡高
					ptt->HuPo_L[2].m=1;//3級邊坡的坡率
					ptt->HuPo_L[2].b=0;//2級邊坡的平台寬度=0(直接與地面相交)
					ptt->HuPo_L[2].style=0;//護坡
 		...
 		else if(TW==1)//路堤
		{
			...
			else if(h0-hNew-Lt_h1>0)//一級護坡沒有與地面相交,高於地面高程,需要繼續多級護坡
			{
				L1=Lt_h1*Lt_m1+L;
				Afla=atan(1.0/Lt_m1);
				Get3DCorrdinate(x1,y1,z1,x2,y2,z2,dx,dz,L1,&x11,&y11,&z11,&x12,&y12,&z12,&x21,&y21,&z21,&x22,&y22,&z22,&mangle);
...
/*
參數說明：
TotalHeight：隧道總高度    Archeight：隧道圓弧高度   WallHeight：隧道立牆高度
Width：隧道寬度   ArcSegmentNumber：隧道圓弧分段數
*/
//求隧道斷面交點坐標和隧道相關信息
...
{
	PRailway3DTunnnel pt;//存儲隧道點
	PTunnnelBridgeInformation tempTunnnelInfor; //存儲隧道信息
	
	double x1,z1,y1;
	double x3,z3,y3;
	
	if(PtS_RailwayLjToBP3D[m_currentSchemeIndexs].GetSize()<=0) //如果當前方案還沒有線路中線數據,返回
		return;
	...
	for (long i=0;i<PtS_RailwayLjToBP3D[m_currentSchemeIndexs].GetSize()-1;i++)
	{
		...
		//如果點類型為"隧道起點","隧道終點"或"隧道中間點",即該點為隧道部分的點
		if(m_style=="隧道起點" || m_style=="隧道終點" || m_style=="隧道中間點") 
		{	
			//左側隧道立牆下方點坐標
			x1=PtS_RailwayLjToBP3D[m_currentSchemeIndexs].GetAt(i)->x1;
			y1=PtS_RailwayLjToBP3D[m_currentSchemeIndexs].GetAt(i)->y1;
			...
			if(m_style=="隧道起點") //如果是"隧道起點"
			{
				pt->Tunnnelindex=index; 
				index++;
				tempTunnnelInfor=new TunnnelBridgeInformation;
				tempTunnnelInfor->startIndex=i+1; //記錄當前隧道的起點在線路中心點的序號
				tempTunnnelInfor->StartLC=PtS_3DLineZX[m_currentSchemeIndexs].GetAt(i)->Lc; //當前隧道的起點里程
				
			}
			
			PtS_Tunnel3D[m_currentSchemeIndexs].Add(pt);//加入隧道左右坐標點數據
			
			float mAngle;
			//計算隧道頂端圓弧的坐標點
			//隧道頂端圓弧實現的方法是,採用橢圓方程根據ArcSegmentNumber內插點數計算組成頂端圓弧各內插點的三維坐標
			//這些各內插點各內插點連接起來,構成隧道頂端圓弧.
			...
			if(m_style=="隧道起點")
			{				
				tempTunnnelInfor->mStartangle=mAngle;//隧道洞門進口角度
			}
			else if(m_style=="隧道終點")
			{
				tempTunnnelInfor->mEndAngle=mAngle; //隧道洞門出口角度
				tempTunnnelInfor->endIndex=i+1;//記錄當前隧道的終點在線路中心點的序號
				tempTunnnelInfor->EndLC=PtS_3DLineZX[m_currentSchemeIndexs].GetAt(i)->Lc;//當前隧道的終點里程
				TunnelInfor[m_currentSchemeIndexs].Add(tempTunnnelInfor);//存儲當前隧道信息
			}

			
			if(m_style=="隧道終點") //如果是"隧道終點"
			{
				pt=new Railway3DTunnnel; 
				pt->x1 =-1;  //-1表示這是隧道出口端
				pt->y1 =-1;  //-1表示這是隧道出口端
				...
				pt->y2 =-1;	 //-1表示這是隧道出口端
				pt->z2 =-1;	 //-1表示這是隧道出口端
				PtS_Tunnel3D[m_currentSchemeIndexs].Add(pt); //存儲隧道數據
			}
...
TotalHeight：隧道總高度    Archeight：隧道圓弧高度   WallHeight：隧道立牆高度
Width：隧道寬度   ArcSegmentNumber：隧道圓弧分段數
(x1,y1,z1):隧道左側立牆底坐標   (x2,y2,z2):隧道右側立牆底坐標
*mAngle:返回相臨隧道斷面的走向
*/
void CDesingScheme::GetTunnelArcCordinate(float TotalHeight, float Archeight, float WallHeight, float Width, int ArcSegmentNumber, double x1, double y1, double z1, double x2, double y2, double z2,float *mAngle)
{
	...
	//(1)求出P2(x2,y2,z2)與P1(x1,y1,z1)組成直線的角度 Peita;
	float dx=x2-x1;
	float dz=z2-z1;
	float Peita;
	...
	*mAngle=Peita;
	...
	for(int i=0;i<=ArcSegmentNumber;i++)
	{
		...
		else if(i==ArcSegmentNumber) //如果是終點
		{
			pt->x=x1;
			pt->y=y1;
			pt->z=z1;
		...
		{
			pt->x=x0+a*cos(Alfa)*cos(Peita); //根據橢圓x坐標計算公式,計算加圓弧內插點x坐標
			pt->y=y0+fabs(b*sin(Alfa));//根據橢圓y坐標計算公式,計算加圓弧內插點y坐標
			if(fabs(x2-x1)<=0.00001) //表示隧道斷面與X軸垂直
...
void CDesingScheme::InertHuPoPts()
{
	...
	for(long i=0;i<PtS_HuPo[m_currentSchemeIndexs].GetSize()-1;i++)
	{
		...
		if(S1*S2<0 && S3*S4>0)//左側路塹與路堤相連,右側為相同類型,需要進行內插處理
		{
			//(1)對左側進行內插
