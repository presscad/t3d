...
void CZdmJtLpPcPD::InitGrid()
{
	...
	m_grid.SetTextMatrix(0,5,"坡度代數差");
...
void CZdmJtLpPcPD::LoadZdmDesignData()
{
	...
	tt.Format("SELECT *  FROM  ZDmJDCurve WHERE 方案名稱='%s' ORDER BY 變坡點樁號",myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename);
	try
	{
...
void CZdmJtLpPcPD::OnButtonDelete() 
{
	...
	if(m_curRow<=0) 	
	{
		MessageBox("請選擇要刪除的記錄!","刪除數據",MB_ICONINFORMATION);
		return;
	}
	...
	for(long i=m_curRow;i<=mEndRow;i++)
	{
		...
		sql.Format("DELETE from ZDmJDCurve WHERE 變坡點樁號=%.3f AND 方案名稱='%s'",lc,myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename);
		...
		catch(_com_error& e)	
		{
			...
			MessageBox(errormessage,"保存實例數據到數據庫",MB_ICONSTOP);
			return;
		} 	
 
		sql.Format("DELETE FROM T3DLineZxCorrdinateZDM WHERE 方案名稱='%s' AND 交點里程=%.3f",\
	...
	MessageBox("刪除成功","刪除數據",MB_ICONINFORMATION);
...
void CZdmJtLpPcPD::SaveData()
{
	...
	for(long i=1;i<=m_totalZDmJD;i++)
	{
		...
		catch(_com_error& e)	
		{
			CString errormessage;
			errormessage.Format("錯誤信息:%s",e.ErrorMessage());
			MessageBox(errormessage,"保存數據",MB_ICONSTOP);
			return;
		} 
		
	}
	MessageBox("保存完成!","保存數據",MB_ICONINFORMATION);
...
void CZdmJtLpPcPD::OnButtonCheck() 
{
	...
	for(i=1;i<=m_totalZDmJD;i++)
	{
		...
		{
			tt.Format("第%d行坡長[%.3f]小於最小坡長【%.3f】",i,m_L,m_minSlopeLength);
			MessageBox(tt,"坡度合法性檢查數據",MB_ICONSTOP);
			mIRow[i]=1;
		}
		if(m_pd>m_maxSlope)
		{
			tt.Format("第%d行坡度[%.3f]大於最大坡度【%.3f】",i,m_pd,m_maxSlope);
			MessageBox(tt,"坡度合法性檢查數據",MB_ICONSTOP);
			mIRow[i]=1;
		}
		if(m_pddsc>m_maxSlopePddsc)
		{
			tt.Format("第%d行坡度差[%.3f]大於最大坡度差【%.3f】",i,m_pddsc,m_maxSlopePddsc);
			MessageBox(tt,"坡度合法性檢查數據",MB_ICONSTOP);
			mIRow[i]=1;
		}	
	}
