...
BOOL CBridgeSet::OnInitDialog() 
{
	...
	//初始化表格控件
	m_gridColor.SetColWidth(0,3000);//設置列寬
	m_gridColor.SetRowHeight(0,400);//設置行高
	m_gridColor.SetRow(0); //設置當前行為第0行
	m_gridColor.SetCol(0);//設置當前列為第0列
	m_gridColor.SetCellBackColor(RGB(m_bridgeColorR,m_bridgeColorG,m_bridgeColorB));//設置背景色
...
void CBridgeSet::OnClickMsflexgrid() 
{
	...
	{
		long m_color=colorDlg.GetColor(); //得到顏色
		m_bridgeColorR=GetRValue(m_color); //紅色份量 
		m_bridgeColorG=GetGValue(m_color);//綠色份量 
		m_bridgeColorB=GetBValue(m_color);//藍色份量 
		m_gridColor.SetRow(0); //設置當前行為第0行
		m_gridColor.SetCol(0);//設置當前列為第0列
		m_gridColor.SetCellBackColor(m_color);//設置背景色
	}	
