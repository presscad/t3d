...
void CTunnelData::InitGrid()
{
	...
	m_grid.SetTextMatrix(0,1,"隧道名稱");
	m_grid.SetTextMatrix(0,2,"起點里程");
	m_grid.SetTextMatrix(0,3,"終點里程");
	m_grid.SetTextMatrix(0,4,"隧道長度");
...
BOOL CTunnelData::CheckValid(BOOL ADDorEDit)
{
	...
	if(ADDorEDit==TRUE) 
		tt.Format("SELECT *  FROM  Tunnel WHERE 方案名稱='%s' AND \
		隧道名稱='%s'",myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename,m_name);
	else
		tt.Format("SELECT *  FROM  Tunnel WHERE 方案名稱='%s' AND \
		  隧道名稱='%s' AND 隧道名稱!='%s'" ,myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename,m_name,m_oldname);
...
void CTunnelData::OnButtonEdit() 
{
	...
	tt.Format("DELETE FROM  Tunnel WHERE 隧道名稱='%s' AND 方案名稱='%s'",\
...
void CTunnelData::WriteToExcel()
{
	...
	try
	{
		...
		sSql = "CREATE TABLE "+sTableFile+" (隧道名稱 TEXT,隧道起點里程  NUMBER,\
			隧道終點里程 NUMBER,隧道長度 NUMBER)";
