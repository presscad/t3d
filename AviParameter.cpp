...
void CAviParameter::OnButtonBrowse() 
{
	...
	if(fd.DoModal ()==IDOK)
	{
		...
		{
			this->MessageBox ("ゅンぃ!","笆礶ゅン把计",MB_OK|MB_ICONERROR);
			return;
		}
	}
...
{
	this->UpdateData(TRUE); //穝跑秖计沮
	
	switch(pScrollBar->GetDlgCtrlID())
	{
	case  IDC_SLIDER_FRAME:
		m_AVIFrame=m_slider_frame.GetPos (); //眔狤れ丁禯玒计
		break;
	case  IDC_SLIDER_MOVIE_WIDTH:
		m_AVIWidth=m_Slider_AVIWidth.GetPos (); //眔AVI糴
		m_MovieWidth=m_AVIWidth;
		break;
	case  IDC_SLIDER_MOVIE_HEIGHT:
		m_AVIHeight=m_Slider_AVIHeight.GetPos (); //眔AVI)蔼
		m_MovieHeight=m_AVIHeight;
		break;
	}
	
	
    this->UpdateData(FALSE);//穝北ン计沮
