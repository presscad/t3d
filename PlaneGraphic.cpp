...
BEGIN_MESSAGE_MAP(CPlaneGraphic, CDialog)
	...
	ON_COMMAND(ID_ZOOMINOUTDYNAMIC, OnZoominoutdynamic)
	ON_COMMAND(ID_GRAPH_MOVE1, OnGraphMove1)
	ON_COMMAND(ID_GRAPH_MOVE3, OnGraphMove3)
	ON_COMMAND(ID_MENU_PLANE_FIRSTSCREEN, OnMenuPlaneFirstscreen)
	ON_COMMAND(ID_MENU_PLANE_PRESCREEN, OnMenuPlanePrescreen)
	ON_COMMAND(ID_MENU_PLANE_NEXTSCREEN, OnMenuPlaneNextscreen)
	ON_COMMAND(ID_MENU_PLANE_OUTCAD, OnMenuPlaneOutcad)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
//選中點為中心縮小/放大
void CPlaneGraphic::OnZoominoutdynamic() 
{
	m_DrawCurrent=10;
}
...
void CPlaneGraphic::OnMenuPlaneFullscreen() 
{
	...
	if(bl1>blc)blc=bl1;  
						 /*以下確定屏幕左下角的實際坐標，即在左側和下側都留了10點陣的區域
	（minx,miny)在屏幕的左下角向右10點陣同時向上10點陣處。*/
...
//移動方式1(移動有連接線)
void CPlaneGraphic::OnGraphMove1() 
{
	m_moveStyle=1;
	m_DrawCurrent=13;
	PushNumb=0; 	
...
//移動方式3(選中點為屏幕中心)
void CPlaneGraphic::OnGraphMove3() 
{
	m_moveStyle=3;
	m_DrawCurrent=13;
	PushNumb=0; 
...
//顯示首屏
void CPlaneGraphic::OnMenuPlaneFirstscreen() 
{
	m_xStart=m_Screen [0].sx;
	m_yStart=m_Screen [0].sy;
	blc=m_Screen [0].blc;	
	this->Invalidate ();	
...
//顯示上一屏
void CPlaneGraphic::OnMenuPlanePrescreen() 
{
	if(m_CurrentScreen>=1)
	{
		m_CurrentScreen--;
		int index=m_wndToolBar.CommandToIndex(ID_MENU_PLANE_PRESCREEN);
		
	}
	
	m_xStart=m_Screen[m_CurrentScreen].sx;
	m_yStart=m_Screen[m_CurrentScreen].sy;
	blc=m_Screen[m_CurrentScreen].blc;	
	this->Invalidate ();	
}

//顯示下一屏
void CPlaneGraphic::OnMenuPlaneNextscreen() 
{
	if(m_CurrentScreen<m_TotalScreen)
		m_CurrentScreen++;
	m_xStart=m_Screen [m_CurrentScreen].sx;
	m_yStart=m_Screen [m_CurrentScreen].sy;
	blc=m_Screen [m_CurrentScreen].blc;	
...
void CPlaneGraphic::LoadCurveData()
{
	...
	m_Schemename=m_currentSchemeNames;
	strsql.Format("select * FROM  Scheme_plane_CureveData WHERE 方案名稱='%s' ORDER BY 交點序號 ASC",m_Schemename);	
	try
	{
	...
	while(!m_Recordset->adoEOF)
	{
		...
		JDCurveElements.Add(pTempCurveElements);
	...
	strsql.Format("select * FROM  Scheme_plane_CureveData_XY WHERE 方案名稱='%s' ORDER BY 交點序號 ASC",m_Schemename);	
	try
	{
		m_Recordset->Open(_bstr_t(strsql),(IDispatch*)(theApp.m_pConnection),adOpenDynamic,adLockOptimistic,adCmdText);    
	}
	...
	while(!m_Recordset->adoEOF)
	{
		pTempCurveElements=JDCurveElements.GetAt(n);
...
void CPlaneGraphic::OnMenuPlaneOutcad() 
{
	CString tt,DxfFilename,mLayerName,mTableName;
	...
	DxfFilename=fielDlg.GetPathName(); 
	if(DxfFilename.IsEmpty())
		return;
	...
	{
		this->MessageBox("文件"+DxfFilename+"已被其它程序打開或佔用,請關閉再試!","輸出到CAD",MB_ICONINFORMATION);
		return;
	}

		
	mdxf.DxfHeader(Dxffile);
	mdxf.DxfLineType(Dxffile);
	mdxf.DxfBeginDrawEnties(Dxffile);
	for(int i=0;i<ployLineCAD.GetSize();i++)
	{
		mdxf.DxfStart_polyline(Dxffile,"圖層polygon",0,m_penWidth[ployLineCAD.GetAt(i)->Index]-1,\
			m_penWidth[ployLineCAD.GetAt(i)->Index]-1,m_penColorIndex[ployLineCAD.GetAt(i)->Index]);
		for(int j=0;j<ployLineCAD.GetAt(i)->PtCAD.GetSize();j++)
		{
			double x=ployLineCAD.GetAt(i)->PtCAD.GetAt(j)->x;
			double y=ployLineCAD.GetAt(i)->PtCAD.GetAt(j)->y;
			mdxf.DxfDraw_PolyLine(Dxffile,"polygon",x,y,0);
		}
		mdxf.DxfEnd_polyline(Dxffile);
	}
	
	for(i=0;i<CircleCAD.GetSize();i++)
	{
		mdxf.DxfDraw_Circle(Dxffile,CircleCAD.GetAt(i)->Cneterx,CircleCAD.GetAt(i)->Cnetery,\
			CircleCAD.GetAt(i)->R,CircleCAD.GetAt(i)->mcolor);
	}
	
	for(i=0;i<TextCAD.GetSize();i++)
	{
		mdxf.DxfDraw_Text(Dxffile,"標注",TextCAD.GetAt(i)->x,TextCAD.GetAt(i)->y,\
			0.8,m_fontSize[TextCAD.GetAt(i)->fontsizeIndex],TextCAD.GetAt(i)->angle,\
			TextCAD.GetAt(i)->strText,2);
	}
	mdxf.DxfSectionEnd(Dxffile);
	tt.Format("dxf文件%s已輸出完成!",DxfFilename);
	MessageBox(tt,"輸出圖形到AuotCAD",MB_ICONINFORMATION);	
}
